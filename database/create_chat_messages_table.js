...
async function createTable() {
  ...
  try {
    ...
    // 读取SQL文件
    const sqlFile = path.join(__dirname, 'chat_messages_table.sql');
    ...
    // 验证表是否创建成功
    const [tables] = await connection.query("SHOW TABLES LIKE 'chat_messages'");
    if (tables.length > 0) {
    ...
    if (err.code === 'ER_TABLE_EXISTS_ERROR' || err.message.includes('already exists')) {
      console.log('ℹ️  chat_messages表已存在，无需重复创建');
    } else {
