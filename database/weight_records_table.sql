...
-- 索引说明：
-- uk_user_date: 唯一索引，确保每个用户每天只有一条记录
-- idx_user_id: 用于快速查询某个用户的所有记录
-- idx_record_date: 用于按日期查询
-- idx_user_date: 用于查询某个用户在特定日期的记录（复合索引）

-- 外键说明：
-- fk_weight_records_user: 关联user表的user_id字段（主键）
-- ON DELETE CASCADE: 当用户被删除时，该用户的所有记录也会被自动删除
-- ON UPDATE CASCADE: 当用户user_id更新时，记录的user_id也会自动更新
